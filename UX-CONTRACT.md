...
## Optional cartoon animation

Party guests can choose **3D cartoon loop** after selecting a photo. Handmade and polished still finishes remain available and handmade remains the default. The third choice uses the same native radio group; selecting it performs no network mutation. The form explains that the output is a repeating five-second video, takes longer, and adds video generation to the host's image-generation API cost. No exact price, duration estimate, or perfectly seamless loop is promised. The choice is disabled with an explanation in local preview, which cannot create live video. If the host changes to a legacy setting before submission, the pending animated choice resets to handmade with a notice; accepted jobs retain their original appearance.

The same UUID protects the entire submitted job. `Job.appearance` and `Job.phase` restore the animation progress after refresh: illustrating creates the colorful 3D portrait; animating creates its video. Progress stays indeterminate. The drawing joins the playable queue only after the video succeeds. A failed or uncertain animation is not presented as a successful still drawing and never automatically starts another paid request.

`Character.mediaType` defaults to image. For video, `assetUrl` identifies the MP4 and `posterUrl` identifies its PNG. Both must use strict same-world `/assets/:id` or `/posters/:id` routes and authenticated fetch-to-blob; tokens never appear in media URLs. `CharacterMedia.tsx` is the shared renderer. It reserves the existing napkin geometry, uses muted autoplay, loop, and playsInline, and displays the poster during loading, blocked autoplay, or playback failure. A manual play/pause or reload control is always named and keyboard operable. Global pause wins over local play; reduced motion prevents automatic playback but permits an explicit play request. Delayed playback resolution cannot restart a paused or unmounted portrait. Stable media URLs preserve playback across snapshot polls; changing the active drawing stops playback, cancels asset requests, and releases blobs. Owner queue thumbnails and legacy scenes use only the poster. Secret answers stay out of media labels until the host reveals them.
